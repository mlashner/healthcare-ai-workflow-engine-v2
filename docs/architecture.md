...
- Observability is a single `AgentRun` with a linear (or lightly branched) trace, which is what the approval UI and eval suite need. Numeric AI metrics (tokens, cost, latency, tool counts) are documented in [`docs/observability.md`](observability.md). A development MCP server for eval and traces is documented in [`docs/mcp.md`](mcp.md).
